...
KIND_CLUSTER_NAME ?= anakosmos-dev
...
backend-build:
	cd backend && go build -o anakosmos-server main.go
...
docker-build:
	podman build -t localhost/anakosmos:latest . || podman build --no-cache -t localhost/anakosmos:latest .

kind-load:
	kind load docker-image localhost/anakosmos:latest --name $(KIND_CLUSTER_NAME)

deploy:
	kubectl apply -f backend/deploy/
	kubectl rollout restart deployment/anakosmos || true
