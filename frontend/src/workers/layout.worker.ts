...
// Layout node/link types for D3 force simulation (extends d3.SimulationNodeDatum)
// x, y, vx, vy are required (d3 initializes them) so simulation types align correctly
...
type SimulationMessage =
  | { type: 'init'; nodes: LayoutNode[]; links: LayoutLink[]; config: LayoutConfig }
  | { type: 'update'; nodes: LayoutNode[]; links: LayoutLink[]; config: LayoutConfig }
  | { type: 'stop' };

const ctx: Worker = self as unknown as Worker;

let simulation: d3.Simulation<LayoutNode, LayoutLink> | null = null;
const nodeStateMap = new Map<string, {x: number, y: number, vx: number, vy: number}>();

// Cache for namespace positions and sizes
const namespacePositionCache: Map<string, { x: number; y: number; radius: number }> = new Map();
 ...
 */
const generateFingerprint = (nodeIds: string[], linkPairs: string[], config: LayoutConfig): string => {
...
ctx.onmessage = (e: MessageEvent<SimulationMessage>) => {
  ...
  if (type === 'init' || type === 'update') {
    ...
    const nodeIds = nodes.map((n: LayoutNode) => n.id);
    const linkPairs = links.map((l: LayoutLink) => `${l.source}-${l.target}`);
    ...
    let clusterScopedCount = rawClusterCount ?? 0;
    ...
    if (enableNamespaceProjection) {
      calculateNamespaceLayout(namespaces || [], namespaceSizes, clusterScopedCount ?? 0);
    }

    // D. Persist previous positions to avoid "jump"
    const mergedNodes: LayoutNode[] = nodes.map((node: LayoutNode) => {
    ...
    const currentIds = new Set(nodes.map((n: LayoutNode) => n.id));
    ...
    simulation = d3.forceSimulation(mergedNodes)
        .force('link', d3.forceLink(links).id((d: d3.SimulationNodeDatum) => (d as LayoutNode).id).distance((link: LayoutLink) => {
        ...
        .force('charge', d3.forceManyBody()
            .strength((d: d3.SimulationNodeDatum) => {
              const n = d as LayoutNode;
              if (n.kind === 'Node') return -100;
        ...
        .force('x', d3.forceX((d: LayoutNode) => {
        ...
        .force('y', d3.forceY((d: LayoutNode) => {
            if (!enableNamespaceProjection) return 0;
            return getNamespaceForce(d.namespace).y;
        }).strength(enableNamespaceProjection ? 0.8 : 0.15))
        
        // Collision detection
        .force('collide', d3.forceCollide((d: LayoutNode) => {
    ...
    const projectionToggled = lastNamespaceProjectionState !== null &&
                              lastNamespaceProjectionState !== undefined &&
                              lastNamespaceProjectionState !== enableNamespaceProjection;
    lastNamespaceProjectionState = enableNamespaceProjection ?? false;
    ...
    simulation?.on('tick', () => {
        ...
        mergedNodes.forEach((node: LayoutNode) => {
            nodeStateMap.set(node.id, { x: node.x ?? 0, y: node.y ?? 0, vx: node.vx ?? 0, vy: node.vy ?? 0 });
        });
        ...
        mergedNodes.forEach((node: LayoutNode) => {
             ...
             positions[node.id] = [node.x ?? 0, height, node.y ?? 0];
        });
