...
export const ResourceLegend: React.FC = () => {
  ...
  const filteredByCategory = useMemo(() => {
    const result: Record<ResourceCategory, KindConfig[]> = {
      workload: [],
      network: [],
      config: [],
      storage: [],
  ...
  const categoryCounts = useMemo(() => {
    ...
    Object.values(KINDS_BY_CATEGORY).forEach((kinds) => {
      kinds.forEach(k => {
        counts[k.category] += kindCounts[k.kind] || 0;
      });
    });
    return counts;
  }, [kindCounts]);
  ...
  // Toggle all kinds in a category
  const toggleCategoryVisibility = (category: ResourceCategory, show: boolean) => {
    const kindsInCategory = KINDS_BY_CATEGORY[category]?.map(k => k.kind) || [];
    if (show) {
      // Remove these kinds from hidden
      setHiddenResourceKinds(hiddenResourceKinds.filter(k => !kindsInCategory.includes(k)));
    } else {
      // Add these kinds to hidden
      const newHidden = [...new Set([...hiddenResourceKinds, ...kindsInCategory])];
      setHiddenResourceKinds(newHidden);
    }
  };
  ...
  const renderKindItem = (item: KindConfig) => {
    ...
    return (
      <div key={item.kind} className="flex flex-col gap-0.5">
        <div className="flex items-center gap-1 group">
            ...
            className={clsx(
                "flex-1 flex items-center gap-2 px-2 py-1 rounded-md text-sm transition-all text-left relative",
                isHidden 
            ...
            <item.icon 
                size={12} 
                className={isHidden ? "text-slate-500" : "text-slate-400"} 
            />
            
            <span className={clsx("flex-1 text-[11px] font-medium truncate", isHidden ? "text-slate-500 line-through decoration-slate-600" : "text-slate-300")}>
                {item.label}
            ...
            <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                {isHidden ? <Square size={10} className="text-slate-600" /> : <CheckSquare size={10} className="text-blue-500" />}
            </div>
            </button>
        ...
        {isKindExpanded && !isHidden && (
            <div className="pl-6 pr-2 py-1 space-y-1 bg-slate-800/30 rounded-b-md border-l-2 border-slate-700/50 ml-2 animate-in slide-in-from-top-1 fade-in duration-200">
                {kindStatuses[item.kind].map(status => {
                    ...
                    return (
                        <button 
                            ...
                            className="w-full flex items-center justify-between px-2 py-0.5 rounded hover:bg-slate-700/50 text-[10px] transition-colors"
                        >
                            <span className={clsx(
                                "truncate",
  ...
  const renderCategorySection = (category: ResourceCategory) => {
    const catConfig = CATEGORY_CONFIG.find(c => c.id === category);
    if (!catConfig) return null;
    
    const kindsInCategory = filteredByCategory[category];
    ...
    // Check if all kinds in this category are hidden
    const allHidden = kindsInCategory.every(k => hiddenResourceKinds.includes(k.kind));
    const someHidden = kindsInCategory.some(k => hiddenResourceKinds.includes(k.kind));
      ...
      <div key={category} className="mb-1">
        <div className="flex items-center gap-1 px-2 py-1.5 group">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              toggleCategoryVisibility(category, allHidden);
            }}
            className={clsx(
              "p-1 rounded opacity-0 group-hover:opacity-100 transition-all",
              allHidden ? "text-slate-600" : someHidden ? "text-slate-400" : "text-blue-400"
            )}
            title={allHidden ? `Show all ${catConfig.label}` : `Hide all ${catConfig.label}`}
          >
        ...
        {isExpanded && (
          <div className="pl-2 space-y-0.5">
            {kindsInCategory.map(renderKindItem)}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="absolute bottom-14 left-4 z-40 flex flex-col items-start max-h-[70vh] transition-all duration-300">
      <div className={clsx(
        ...
        isExpanded ? "w-72" : "w-10 h-10 rounded-full"
      )}>
          ...
          <div className="overflow-y-auto custom-scrollbar flex-1 p-1">
            ...
            <div className="px-2 py-1.5 border-b border-slate-700/50">
              ...
              >
                  ...
                  <MapIcon size={12} />
                  <span className="text-[11px]">Namespace Zones</span>
                  <div className="ml-auto">
                    {enableNamespaceProjection ? <CheckSquare size={10} className="text-blue-500" /> : <Square size={10} className="text-slate-600" />}
                  </div>
              </button>
            </div>
            ...
            <div className="px-2 py-1.5 border-b border-slate-700/50">
               ...
               <div className="flex flex-col gap-0.5">
                  <button 
                    ...
                    className={clsx(
                        "flex items-center gap-2 text-[11px] transition-colors w-full text-left py-0.5",
            ...
            <div className="py-1.5">
              {filteredBySearch.length === 0 ? (
                ...
                <>
                  {renderCategorySection('workload')}
