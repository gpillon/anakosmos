...
export const DeploymentView: React.FC<DeploymentViewProps> = ({ resource, activeTab }) => {
  ...
  // This ensures tabs re-initialize their local state when resource changes
  ...
  useEffect(() => {
    if (resource.raw) {
      setFetchedRaw(null); // Clear fetched raw since resource.raw is now available
    }
  }, [resource.raw]);
  ...
  // Apply changes helper for sub-components - no need for manual refresh, watcher handles it
  const applyChanges = async (updatedRaw: any) => {
    if (!client) return;
    try {
      const yamlStr = yaml.dump(updatedRaw);
      await client.applyYaml(resource.namespace, resource.kind, resource.name, yamlStr);
      // Watcher will update the store automatically, which updates resource prop
    } catch (e) {
  ...
  // Create a resource object with the raw data for passing to tabs
    ...
    <div className="h-full overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
      <div className="p-6 max-w-6xl mx-auto">
        ...
        {isLoading ? (
          ...
          <>
            {/* Key forces re-mount when resource version changes, resetting local states */}
